...
Then(/^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails?$/) do |address, amount|
  expect(unread_emails_for(address).count).to eql parse_email_count(amount)
end

Then(/^(?:I|they|"([^"]*?)") should have (an|no|\d+) emails?$/) do |address, amount|
  expect(mailbox_for(address).count).to eql parse_email_count(amount)
end

Then(/^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with subject "([^"]*?)"$/) do |address, amount, subject|
  expect(unread_emails_for(address).select { |m| m.subject =~ Regexp.new(Regexp.escape(subject)) }.count).to eql parse_email_count(amount)
end

Then(/^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with subject \/([^"]*?)\/$/) do |address, amount, subject|
  expect(unread_emails_for(address).select { |m| m.subject =~ Regexp.new(subject) }.count).to eql parse_email_count(amount)
end
...
Then(/^(?:I|they) should see (an|no|\d+) attachments? with the email$/) do |amount|
  expect(current_email_attachments.count).to eql parse_email_count(amount)
end

Then(/^there should be (an|no|\d+) attachments? named "([^"]*?)"$/) do |amount, filename|
  expect(current_email_attachments.select { |a| a.filename == filename }.count).to eql parse_email_count(amount)
end
...
Then(/^there should be (an|no|\d+) attachments? of type "([^"]*?)"$/) do |amount, content_type|
  expect(current_email_attachments.select { |a| a.content_type.include?(content_type) }.count).to eql parse_email_count(amount)
end
...
Then(/^all attachments should not be blank$/) do
  current_email_attachments.each do |attachment|
    expect(attachment.read.count).to_not eql 0
  end
end
